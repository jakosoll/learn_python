...
Output: Int.
"""
dict_of_diff = {i: abs(one - j) for i, j in enumerate(sorted(values))}
    index_difference: tuple = sorted(dict_of_diff.items(), key=lambda item: item[1])[0]
    return list(sorted(values))[index_difference[0]]
